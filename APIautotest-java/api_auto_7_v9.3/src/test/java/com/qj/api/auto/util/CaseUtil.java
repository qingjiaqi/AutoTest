...
public class CaseUtil {
	...
	//保存另外一个sheet页 用例2的数据
	public static List<Case> cases2=new ArrayList<Case>();
	//通过静态代码块保证数据都封装进了cases对象中
	static{
		
		List<Case> listCase=ExcelUtil.load(propertiesUtil.getExcelPath("excel.PathName"),"用例",Case.class);
		//将list集合添加到cases集合中去
		cases.addAll(listCase);
		
		List<Case> listCase2=ExcelUtil.load(propertiesUtil.getExcelPath("excel.PathName"),"用例2",Case.class);
		//将list集合添加到cases集合中去
		cases2.addAll(listCase2);
		
		
		
	}
	/**通过这个方法获取到“用例sheet页”中指定接口编号的指定列数据
	 * @param apiId 接口编号
	...
	/**通过这个方法获取到“用例2sheet页”中指定接口编号的指定列数据
	 * @param apiId 接口编号
	 * @param cellNames 要获取的列的列名
	 * @return 返回我们获取的数据 Object[][]
	 */
	public static Object[][] getCaseDateByApiId2(String apiId,String[] cellNames){
		//获取我们case的字节码文件
		Class clazz=Case.class;
	...
	public static void main(String[] args) {
		...
		Object[][] datas=getCaseDateByApiId2("1", a);
