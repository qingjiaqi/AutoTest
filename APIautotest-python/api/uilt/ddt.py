...
# These attributes will not conflict with any real api attribute
...
def mk_test_name(name, value, index=0):
    ...
    api identifier by replacing extraneous characters with ``_``.
...
def ddt(cls):
    ...
    valid api identifier.  If ``data.__name__`` exists, we use that instead.

    For each method decorated with ``@file_data('test_data.json')``, the
    decorator will try to load the test_data.json file located relative
    to the api file containing the method that is decorated. It will,
